...
### 特殊数据结构：单调队列 Mono Queue

队列中的元素递增或递减

例子：给定一个数组，有一个大小为k的华东窗口从数组的最左侧移动到最右侧。只可以看到在滑动窗口k内的数字。滑动窗口每次只向右移动一位。返回滑动窗口最大值

例子：input nums = [1, 3, -1, -3, 5, 3, 6, 7], k = 3; ouptut: [3, 3, 5, 5, 6, 7];

在一堆数字中，已知最值，如果给这堆数添加一个数，那么比较一下就可以很快算出最值；但如果减少一个数（减少的是最值），就不一定能很快得到最值了，而要遍历所有数重新找最值。
