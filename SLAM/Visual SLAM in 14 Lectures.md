...
$$\begin{cases} x_k=f(x_{k-1}, u_k)+w_k \\ z_{k,j} = h(y_j, x_k)+v_{k,j} \mapsto sz_{k,j} = K exp(\xi^\wedge)y_j\end{cases}$$
...
传统上EKF关系当前时刻的状态估计$x_k$而对之前的状态不多考虑；近年来，非线性优化方法，使用所有时刻采集到的数据进行状态估计，被认为优于传统的滤波器。



在非线性优化中，把所有待估计的变量放在一个“状态变量”中，在已知输入数据$u$和观测数据$z$的条件下，计算状态$x$的条件概率分布$P(x|z, u)$。特别地，当没有测量运动的传感器，而只有一张张的图像时，即只考虑观测方程带来的数据时，相当于估计$P(x|z)$的条件概率分布。如果忽略图像在时间上的联系，该问题也称为Structure from Motion (SfM)，即如何从许多图像中重建三维空间结构。SLAM可以看作图像具有时间先后顺序，需要实时求解一个SfM问题。利用贝叶斯法则
